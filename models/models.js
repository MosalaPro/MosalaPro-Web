  ...
  req.login(user, function (err) {
    if (err) {
      ...
      passport.authenticate("local")(req, res, function () {
        ...
        else console.log("User has not been verified");
        res.redirect("/");
      });
    }
  });
...
exports.showHomePage = function (req, res) {
  ...
  } else res.render("home", { usr: null, cats: categories });
};
  ...
  try {
    ...
    await User.register(newUser, req.body.password, function (err, u) {
      if (err) {
        ...
        // TODO: activate error message on modal
      } else console.log("User has been successfully registered.");
      //res.redirect("/");
    });
...
exports.showForProPage = function (req, res) {
  ...
  } else res.render("forProfessionals", { usr: null });
};

exports.showFindProfessionalsPage = function (req, res) {
  if (req.isAuthenticated()) res.render("findprofessionals", { usr: req.user });
  else res.render("findprofessionals", { usr: null });
};
  ...
  if (true) {
    try {
      ...
      res
        ...
        .send({
          responseCode: 400,
          responseMessage: "Error posting service request",
        });
    }
  } else res.render("forProfessionals", { usr: null });
};
...
global.sendEmail = async function sendEmail(
  name,
  email,
  subject,
  message,
